...
def synthesize(
    runs: RunStore, run_id: str, script: Script, engine: TTSEngine, voice: str, speed: float
) -> list[NarrationClip]:
    ...
    for scene in script.scenes:
        ...
        if missing:
            _synthesize_to(destination, scene.narration, engine, voice, speed)
        clips.append(NarrationClip(scene.id, clip_duration_seconds(destination), missing))
    return clips


def _synthesize_to(destination: Path, narration: str, engine: TTSEngine, voice: str, speed: float) -> None:
    """Publish by rename, so an interrupted clip is never mistaken for a finished one on resume."""
    partial = destination.with_name(destination.name + ".part")
    try:
        engine.synthesize(narration, voice, speed, partial)
    except TTSError:
        raise
